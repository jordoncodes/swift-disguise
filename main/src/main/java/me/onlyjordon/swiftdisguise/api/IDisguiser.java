...
public abstract class IDisguiser {

    protected HashMap<UUID, DisguiseData> data = new HashMap<>();

    @Nullable
    public IDisguiseData getData(Player player) {
    ...
    public void resetSkin(@NotNull Player player) {
        ...
        if (skin == null) return;
        if (setSkin(player, skin)) d.setCurrentSkin(null);
    }
    ...
    public abstract void setSkinLayers(Player player, SkinLayers layers);
